...
$cmd = "select * from Resgate order by data DESC limit $inicio,$registros";
...
if ($total > 0) {
    ...
    while ($objResgate = mysql_fetch_object($resgate)) {
        ...
        $idResgate = base64_encode($objResgate->idResgate);
        ...
        if ($objResgate->status == 0) {
            ...
            echo "<td>		
							<a title='Imprimir Recibo de Confirmação de Resgate' target='_blank' href='../recibos/confirmarResgate.php?idResgate=$objResgate->idResgate'><i class='glyphicon glyphicon-print'></i></a>
							&nbsp;
    ...
    for ($i = 1; $i < $numPaginas + 1; $i++) {
        echo " <li><a style='text-align:center;' href='amigoseResgates.php?pagina=$i&filter=padrao'>" . $i . "</a></li>";
    }
    echo "</ul>";
} else {
